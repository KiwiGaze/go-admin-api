...
func Authenticator(c *gin.Context) (interface{}, error) {
	...
	if err != nil {
		...
		response.Error(c, 500, err, "failed to get database connection")
	...
	if err = c.ShouldBind(&loginVals); err != nil {
		...
		msg = "failed to parse request data"
		status = "1"
...
// @Accept  application/json
// @Product application/json
// @Success 200 {string} string "{"code": 200, "msg": "logout successful" }"
// @Router /logout [post]
// @Security Bearer
func LogOut(c *gin.Context) {
	LoginLogToDB(c, "2", "logout successful", user.GetUserName(c))
