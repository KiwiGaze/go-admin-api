...
import (
	...
	cModels "go-admin-api/common/models"
...
func (e *SysMenu) Get(d *dto.SysMenuGetReq, model *models.SysMenu) *SysMenu {
	...
	if err != nil && errors.Is(err, gorm.ErrRecordNotFound) {
		err = errors.New("object does not exist or no permission to view")
...
// Insert creates a SysMenu object
...
func (e *SysMenu) initPaths(tx *gorm.DB, menu *models.SysMenu) error {
	...
	if menu.ParentId != 0 {
		...
		if parentMenu.Paths == "" {
			err = errors.New("parent paths are invalid, please try updating the parent menu of the current node")
			return err
		}
...
// Update updates the SysMenu object
func (e *SysMenu) Update(c *dto.SysMenuUpdateReq) *SysMenu {
	...
	if db.RowsAffected == 0 {
		_ = e.AddError(errors.New("no permission to update this data"))
		return e
	}
...
// SetLabel sets the menu base data used when editing roles
...
// GetSysMenuByRoleName returns the left-side menu for the given roles
...
// menuLabelCall recursively builds the menu label tree
...
// SetMenuRole returns the left-side menu tree for a role
