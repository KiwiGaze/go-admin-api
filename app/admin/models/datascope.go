...
func (e *DataPermission) GetDataScope(tableName string, db *gorm.DB) (*gorm.DB, error) {
	if !config.ApplicationConfig.EnableDP {
		usageStr := `Data permission is currently ` + pkg.Green(`disabled`) + ` for you. To enable it, refer to the configuration file field description`
		log.Debug("%s\n", usageStr)
		return db, nil
	}
	user := new(SysUser)
	...
	if err != nil {
		return nil, errors.New("failed to retrieve user data, msg: " + err.Error())
	}
	err = db.Find(role, user.RoleId).Error
	if err != nil {
		return nil, errors.New("failed to retrieve role data, msg: " + err.Error())
	}
	if role.DataScope == "2" {
