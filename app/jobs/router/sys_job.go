...
import (
	"go-admin-api/app/jobs/apis"
	models2 "go-admin-api/app/jobs/models"
	dto2 "go-admin-api/app/jobs/service/dto"
	"go-admin-api/common/actions"
	"go-admin-api/common/middleware"
...
// registerSysJobRouter registers authenticated routes.
func registerSysJobRouter(v1 *gin.RouterGroup, authMiddleware *jwt.GinJWTMiddleware) {
	...
	r := v1.Group("/sysjob").Use(authMiddleware.MiddlewareFunc()).Use(middleware.AuthCheckRole())
	{
		...
		r.DELETE("", actions.PermissionAction(), actions.DeleteAction(new(dto2.SysJobDeleteReq)))
	}
	sysJob := apis.SysJob{}

	job := v1.Group("/job").Use(authMiddleware.MiddlewareFunc()).Use(middleware.AuthCheckRole()).Use(actions.PermissionAction())
	{
		job.GET("/remove/:id", sysJob.RemoveJobForService)
		job.GET("/start/:id", sysJob.StartJobForService)
	}
}
