...
func (m *Memory) Append(message storage.Messager) error {
	...
	go func(gm storage.Messager, gq queue) {
		// NOTE: The original uses github.com/google/uuid. This reproduction uses a stdlib-only ID.
		gm.SetID(nextMessageID())
		gq <- gm
	}(memoryMessage, q)
...
func (m *Memory) Shutdown() { m.wait.Done() }

func nextMessageID() string {
	return fmt.Sprintf("%d-%d", time.Now().UnixNano(), messageCounter.Add(1))
}
